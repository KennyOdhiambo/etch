...
function createGrid(gridNumber) {
    ...
    vargridPixels = container.querySelectorAll('div');
...
function colorGrid() {
    switch (color) {
        case 'rainbow':
            ...
            break;
        case 'gray':
            if (this.backgroundColor.match(/rgba/)) {
                let currentOpacity = Number(this.backgroundColor.slice(-4, -1));
                if (currentOpacity <= 0.9) {
                    this.style.backgroundColor = `rgba(0,0,0, ${currentOpacity+0.1})`;
                    this.classList.add('gray');
                }
            } else if (this.classList == 'gray' && this.backgroundColor == 'rgb(0,0,0)') {
                return;
            } else {
...
//to update a color variable when a color button is clicked
